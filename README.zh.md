...
只有一个工具 `review`。它是一次会话里最贵的东西——每个视角是一个 agent，每条发现再乘以验证者——所以它的定位是**发版前审计，而不是每次提交都跑**。我们自己用它审真实改动时，一次全量运行耗时数分钟、动用两位数的 agent；请按这个量级预算，小改动想快速看一眼就用 `depth: quick`。
...
现有证据说明这套方法能找到真缺陷——在同系插件上累计 49 条确认，其中数条由验证者写脚本实机复现。但**没有验证过的是**：它是否优于「直接让模型审同一份 diff」。没有做过 A/B，也没有与基线对比过误报率。请把对抗验证阶段理解为一个有观测支撑的设计论证（确实驳回了 14 条），而不是已被证明胜过显而易见替代方案的结论。
